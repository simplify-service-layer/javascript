...
interface ServiceBase {
  constructor: typeof ServiceBase;
...
abstract class ServiceBase {
  ...
  public abstract hasArrayObjectRuleInRuleList(ruleList: any[]): boolean;

  public abstract removeDependencyKeySymbolInRule(rule: any): any;
  ...
  > {
    const self = this;
    let arr: Map<typeof ServiceBase, { [key: string]: any[] }> = new Map();
  ...
  public static getAllTraits(): (typeof ServiceBase)[] {
    const self = this;
    let arr: (typeof ServiceBase)[] = [];
  ...
  public static getTraits(): (typeof ServiceBase)[] {
    return [];
  }
        ...
        for (let i = 0; i < segs.length - 1; ++i) {
          ...
          this.constructor.getAllRuleLists().forEach((parentRuleLists, cl) => {
            let parentRuleList = _.has(parentRuleLists, parentKey)
              ...
              : [];
            if (cl.prototype.hasArrayObjectRuleInRuleList(parentRuleList)) {
      ...
      while (allSegs) {
        ...
        if (_.isArray(rKeyVal) && !_.has(rKeyVal, seg)) {
          ruleLists[k] = _.filter(ruleLists[k], (rule) => {
            return cls.prototype.filterPresentRelatedRule(rule);
          });
        }
    ...
    _.forEach([...this.constructor.getAllTraits(), self.constructor], (cls) => {
      ...
      _.forEach(ruleLists, (ruleList, k) => {
        _.forEach(ruleList, (rule, i) => {
          const presentRelatedRule = this.filterPresentRelatedRule(rule);
